...
GNU General Public License.  A copy of the GNU license is included
...
character ('-') is given as the filename.
...
\begin{description}
\item[7 ] Copy the current HDU from the FITS file associated with infptr and append it
...
6)  imcopy 'infile.fit+1[1001:1500,2001:2500]'  outfile.fit
...
7)  imcopy 'infile.fit+1[1001:1500,2001:2500]'  outfile.fit.gz
...
\item[1 ] Insert or delete rows in an ASCII or binary table. When inserting rows
   ...
   number of rows in the table. \label{ffirow} \label{ffdrow} \label{ffdrws}
\end{description}
...
`Extended File Name Syntax' chapter of this document.
...
\begin{description}
\item[17]  Check that the Header fill bytes (if any) are all blank.  These are the bytes
...
\begin{description}
\item[18]  Check that the Data fill bytes (if any) are all zero (for IMAGE or BINARY Table HDU)
    or all blanks (for ASCII table HDU).  These file bytes may be
   located after the last valid data byte in the HDU and before the physical end of the HDU.
     \label{ffcdfl}
\end{description}
...
\begin{description}
\item[8 ]Flush any internal buffers of data to the output FITS file. This
   routine rarely needs to be called, but can be useful when writing to
   the FITS files in memory.  It also ensures
...
currently set to 25 by default.
