...
On OS/2 systems, CFITSIO can be built by typing 'make -f makefile.os2'.
This makefile requires the GCC compiler and EMX library,
which are available from many Internet sites containing OS/2
software, such as ftp-os2.nmsu.edu/pub/os2/dev/emx/v0.9c
and ftp.leo.org/pub/comp/os/os2/leo/devtools/emx+gcc.
...
\begin{description}
\item[10] Modify the size, dimensions, and/or datatype of the current
...
\begin{description}
\item[11] Delete the CHDU in the FITS file.  Any following HDUs will be shifted
...
\begin{description}
\item[12] Copy the entire CHDU from the FITS file associated with infptr to the CHDU
...
\begin{description}
\item[13] Copy the data (and not the header) from the CHDU associated with infptr
...
\begin{description}
\item[14] This routine forces CFITSIO to scan the current header keywords that
...
\item[11] Append the physical units string to an existing keyword.  This
    ...
    field.  This routine is currently experimental, and this local
   convention may change in future releases of CFITSIO.
\end{description}
...
\item[10] Get the physical units string in an existing keyword.  This
    ...
    for the keyword.  This routine is currently experimental, and this local
   convention may change in future releases of CFITSIO.
\end{description}
