...
Starting with version 1.4, CFITSIO now also contains a set of Fortran
callable wrapper routines which allow Fortran programs to call the
CFITSIO routines.  See the companion ``FITSIO User's Guide'' for the
definition of the Fortran subroutine calling sequences.  These wrappers
replace the older Fortran FITSIO library which is no longer supported.
...
\begin{description}
\item[2 ]  Create a new empty FITS file. \label{ffinit}
\end{description}
...
\begin{description}
\item[4 ] Close a previously opened FITS file. \label{ffclos}
\end{description}
...
\begin{description}
\item[5 ]Return the name of the opened FITS file. \label{ffflnm}
\end{description}
...
\item[1 ] Return the number of existing keywords (not counting the
   ...
   limit to the number of keywords that can be added to a header. \label{ffghsp}
\end{description}
...
\item[7 ] Write the DATE keyword to the CHU. The keyword value will contain
   ...
   problem and will be implemented in CFITSIO in late 1998. \label{ffpdat}
\end{description}
...
\begin{description}
\item[9 ] Write the physical units string into an existing keyword.  This
...
\begin{description}
\item[10]  Rename an existing keyword preserving the current value
...
\begin{description}
\item[11]  Modify (overwrite) the comment field of an existing keyword. \label{ffmcom}
\end{description}
...
\begin{description}
\item[12] Read the nth header record in the CHU.  The first keyword
...
\begin{description}
\item[13]  Read the header record having the specified keyword name. \label{ffgcrd}
\end{description}
...
\begin{description}
\item[14] Read a specified keyword value and comment.
...
\begin{description}
\item[15] Read the physical units string in an existing keyword.  This
    routine uses a local convention, shown in the following example,
    in which the keyword units are enclosed in square brackets in the
...
\begin{description}
\item[16] Delete a keyword record.  The space previously occupied by
...
\begin{description}
\item[1 ] Open a FITS file residing in core computer memory.  This routine
...
\begin{description}
\item[2 ]Flush any internal buffers of data to the output FITS file. This
...
\begin{description}
\item[3 ] Close and DELETE a FITS disk file previously opened with ffopen or ffinit.
...
\item[1 ] Get the byte offsets in the FITS file to the start and end of the CHDU.
   ...
   is also equal to the size of the entire FITS file. \label{ffghad}
\end{description}
...
\item[2 ] Create (append) a new empty HDU at the end of the FITS file.
   ...
   no header keywords. \label{ffcrhd}
\end{description}
...
\item[1 ] Write the primary header or IMAGE extension keywords into the CHU.
 ...
 EXTEND parameters are ignored. \label{ffphpr} \label{ffphps}
\end{description}
...
\begin{description}
\item[1 ] Update an 80-character record in the CHU.  \label{ffucrd}
\end{description}
...
\begin{description}
\item[3 ]Set data array elements as undefined. \label{ffppru}
\end{description}
...
\begin{description}
\item[5 ] Write a 2-D image into the data array. \label{ffp2dx}
\end{description}
...
\begin{description}
\item[7 ] Write a 3-D cube into the data array. \label{ffp3dx}
\end{description}
...
\begin{description}
\item[8 ]  Write an arbitrary data subsection into the data array. \label{ffpssx}
\end{description}
...
\section{Advanced General Utility Routines}
...
\item[1 ] Get the current system date.  C already provides standard
   ...
   the Fortran FITSIO library). \label{ffgsdt}
\end{description}
...
\begin{description}
\item[3 ]  Clear the entire error message stack.  This routine is useful
...
\begin{description}
\item[4 ] Convert a character string to uppercase (operates in place). \label{ffupch}
\end{description}
...
\begin{description}
\item[5 ]  Compare the input template string against the reference string
...
\begin{description}
\item[6 ] Test that the keyword record contains only legal printable ASCII characters
   \label{fftrec}
\end{description}
...
\begin{description}
\item[7 ] Parse a header keyword record.
...
\begin{description}
\item[11] Parse the 'TFORM' binary table column format string.
...
\begin{description}
\item[12] Parse the 'TFORM' keyword value that defines the column format in
...
\begin{description}
\item[13] Calculate the starting column positions and total ASCII table width
...
\begin{description}
\item[14] Parse a template string and return a formatted 80-character string
...
\begin{description}
\item[16] Check that the Header fill bytes (if any) are all blank. \label{ffchfl}
\end{description}
...
\begin{description}
\item[17] Check that the Data fill bytes (if any) are all zero or blank. \label{ffcdfl}
\end{description}
...
\begin{description}
\item[18]  Create or parse the FITS format date and time string.  Starting in
the year 2000, the FITS DATE keyword values (and the values of other
`DATE-' keywords) must have the form 'YYYY-MM-DD' (date only) or
'YYYY-MM-DDThh:mm:ss.dddd' (date and time) where the number of decimal
places in the seconds value is optional.  These times are in UTC.  The
older 'dd/mm/yy' date format may not be used for dates after 01 January
2000.

The following routines help to construct or parse these FITS date/time
strings.  The first routine constructs a new-format date string, while
the second routine constructs a new-format date+time string.  The 3rd
and 4th routines return the date and time as read from the input
string, where the string may be in either the old or new format.
The time is set to zero if it is not included in the string.
   \label{ffdt2s}
\end{description}

\begin{verbatim}
int fits_date2str/ffdt2s   (construct 'YYYY-MM-DD' string)
    (int year, int month, int day, > char *datestr, int *status)

int fits_time2str/fftm2s   (construct 'YYYY-MM-DDThh:mm:ss.dddd' string)
    (int year, int month, int day, int hour, int minute, double second,
     int decimals, > char *datestr, int *status)

int fits_str2date/ffs2dt  (read the date from the string)
    (char *datestr, > int *year, int *month, int *day, int *status)

int fits_str2time/ffs2tm  (read date and time from the string)
