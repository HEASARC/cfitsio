...
CFITSIO depends upon the availability of the libcurl library (with development
files) in order to perform HTTPS file access.  The CFITSIO 'configure' script will search
for this library on your system, and if it finds it it will automatically be incorporated
into the build.  You can tell if it was found by the the presence of '-DCFITSIO\_HAVE\_CURL=1' 
in the CFITSIO Makefile and in the compilation output.  If 'configure' is unable to find a usable libcurl, CFITSIO will still
successfully build but won't have HTTPS capability.
...
CFITSIO has 3 functions which specifically apply to HTTPS access:  fits\_init\_https, 
fits\_cleanup\_https, and fits\_verbose\_https.  These are described in the 'FITS File Access Routines'
section of the 'Specialized CFITSIO Interface Routines' chapter. 
