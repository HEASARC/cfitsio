...
\item[4 ]  Create a new FITS file, using a template file to define its
  ...
  under the fits\_parse\_template routiene (in the general Utilities
...
\begin{description}
\item[8 ] Parse the input filename or URL into its component parts: the file
...
\begin{description}
\item[9 ] Parse the input filename and return the extension number that would
be moved to if the file were opened with fits\_open\_file.  If an
extension name is included in the file name specification (e.g.
"myfile.fits[EVENTS]" then this routine will have to open the FITS file
and look for the position of the named extension.  This routine is mainly
...
efficient to open the FITS file with fits\_open\_file, then use
fits\_get\_hdu\_num to determine which HDU in the file has been
opened.
 \label{ffextn}
\end{description}
...
\begin{description}
\item[10] Parse the input file name and return the root file name.  The root
