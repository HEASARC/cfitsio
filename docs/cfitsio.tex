...
The CFITSIO library file, libcfitsio.a, and 2 include files, fitsio.h
and longnam.h, should then be copied to the appropriate directory(s)
for use when compiling application programs.  Alternatively, one could
set 2 environment variables, FTOOLS\_LIB and FTOOLS\_INCLUDE, to
specify the paths to the installation directories, (e.g., `setenv
FTOOLS\_INCLUDE /usr/local/include') and then run `make install' to
...
CFLAGS line, then rebuild the library with 'make'.  Finally, build the
shared library with
...
make clean, undefine HAVE\_NET\_SERVICES on the CFLAGS line and
rebuild.  It's unimportant whether or not -fPIC is used when building
the static library.
...
\begin{description}
\item[2 ] Write pixels into the FITS data array.  'fpixel' is an array of
...
\begin{description}
\item[3 ] Set FITS data array elements equal to the appropriate null pixel
...
\begin{description}
\item[4 ] Read a rectangular subimage from the FITS data array.  The fpixel
    and lpixel arrays give the coordinates of the first (lower left
    corner) and last (upper right corner) pixels to be included in the
    subimage Undefined FITS array elements will be returned with a
    value = *nullval, (note that this parameter gives the address of
    the null value, not the null value itself) unless nulval = 0 or
    *nulval = 0, in which case no checks for
...
\begin{description}
\item[5 ] Read pixels from the FITS data array.  'fpixel' is the starting
