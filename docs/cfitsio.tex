...
column, CFITSIO recognized 3 additional datatype codes besides those
...
for a signed byte column.  These non-standard datatype codes are not
...
`array' pointers and can have one of the following  values:  TBYTE, TSBYTE,
...
\begin{description}
\item[2 ]Create a new primary array or IMAGE extension with a specified
...
\begin{description}
\item[3 ] Write a rectangular subimage (or the whole image) to the FITS data
...
\begin{description}
\item[4 ] Write pixels into the FITS data array.  'fpixel' is an array of
...
\begin{description}
\item[5 ] Set FITS data array elements equal to the appropriate null pixel
...
\begin{description}
\item[6 ] Read a rectangular subimage (or the whole image) from the FITS
...
\begin{description}
\item[7 ] Read pixels from the FITS data array.  'fpixel' is the starting
...
tables support the following datatype values:  TSTRING, TBYTE, TSBYTE, TSHORT,
...
\item[5 ] Read bit values from a byte ('B') or bit (`X`) table column (in the
    ...
    of 8 bits where the bit array starts with the most significan bit
...
datatype - specifies the datatype of the value.  Allowed value are:  TSTRING,
