...
Jurek Borkowski also wrote the template parsing routines.
...
is included as part of the input file specification.  The local machine
must then be able to hold a copy of the whole FITS file in memory,
otherwise CFITSIO will exit with an error.  If this is a problem (e.g.,
on machines with a small amount of memory, or when reading very large
files) then the output file specifier may be used to give the name of a
local disk file that CFITSIO will create and use instead of trying to
...
then the entire line (without the first 8 spaces) is treated as a
comment and copied verbatim into the FITS header. Thus lines can be
indented, but indentation is limited to a maximum of 7 spaces.
...
\begin{itemize}
\item
 $\backslash${\tt include} - must be followed by a filename. Forces parser to temporarily
	...
	span multiple files.
\item
 $\backslash${\tt group} - marks beginning of GROUP definition
\item
 $\backslash${\tt end} - marks end of GROUP definition
\end{itemize}
...
\subsubsection{Auto-indexing of Kewyords}

If a template keyword name ends with a "\#" character, it is
auto-indexed. The parser replaces the '\#' with the current integer
keyword index value in the
FITS header. The first keyword field encountered in a given HDU
definition ending with '\#' is used as the index value incrementor.
Each time this keyword field (also ending with '\#') is encountered in
the header definition the index value is incremented by 1.
