...
\item[10 ] Wrapper functions for global initialization and cleanup of the libcurl library used
   ...
   should be called after all HTTPS file accessing is completed, and after all threads are 
...
\begin{verbatim}
  ...
  -#VEL;
\end{verbatim}
It is also possible to use wildcard expressions to remove any keyword
that matches a pattern.  Wildcard patterns are: \verb+'*'+, which matches any
...
\end{verbatim}
will remove any keyword beginning with 'DEC\_' and any
keyword beginning with 'TUNIT' and ending with a number.

However, the above expressions will only delete the {\it first}
keyword which matches the pattern.  If multiple keywords later
...
\end{verbatim}
Note that this form will succeed if the requested keyword is not
present.
