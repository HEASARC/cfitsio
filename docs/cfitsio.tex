...
\begin{description}
\item[5 ] Write pixels into the FITS data array.  \label{ffppx}
\end{description}
 
\begin{verbatim}
  int fits_write_pix / ffppx
      (fitsfile *fptr, int datatype, long *firstpix, long nelements,
...
\item[6 ]Write pixels into the FITS data array, substituting the appropriate
  ...
  ignored and this routine behaves the same as fits\_write\_pix. \label{ffppxn}
\end{description}
...
\item[9 ] Read pixels from the FITS data array.   Undefined FITS array
    ...
    parameter gives the address of the null value, not the null value
    itself) unless nulval = 0 or *nulval = 0, in which case no checks for
   undefined pixels will be performed. \label{ffgpxv}
\end{description}
...
firstpix - Coordinate of the first pixel to be read or written in the
           FITS array.  The number of elements in this array must = NAXIS.
           First pixel in each dimension = 1, and the coordinates are
           given in order of the FITS dimensions, i.e., the first element
           of the firstpix array gives the coordinate of the pixel along
           the NAXIS1 axis.
