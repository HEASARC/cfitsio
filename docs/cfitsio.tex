...
CFITSIO now supports an extended input filename syntax of the form

\begin{verbatim}
   filetype://filename.ext[extension][rowfilter][binning][columns]
\end{verbatim}
See the file README.filenames for further information.
...
\item[2 ] Reopen a FITS file that was previously opened with
    ...
    simultaneously read or write to 2 different extensions in
    the same file.
...
\begin{description}
\item[3 ]  Create a new empty FITS file. \label{ffinit}
\end{description}
...
\begin{description}
\item[6 ]Return the name of the opened FITS file. \label{ffflnm}
\end{description}
...
\begin{description}
\item[7 ]Return the I/O mode of the open FITS file (READONLY or READWRITE).
\label{ffflmd}
\end{description}
...
\begin{description}
\item[8 ]Parse the input filename and return the extension number that
would be moved to if the file were opened with fits\_open\_file.
 \label{ffextn}
\end{description}
...
2000.  The following routines help to construct or parse these FITS
 date/time strings. \label{ffdt2s}
\end{description}
...
Construct a new-format date string ('YYYY-MM-DD') from the
input date values.
...
Construct a new-format date+time string ('YYYY-MM-DDThh:mm:ss.dddd').
If the year, month, and day values all = 0 then only the time is returned
with format 'hh:mm:ss.dddd');
...
in either the old or new FITS format.  Null pointers may be supplied
for any unwanted return date parameters.
...
string may be in either the old or new format.  The returned hours,
minutes, and seconds values will be set to zero if the input string
does not include the time.  Similarly, the returned year, month, and
date values will be set to zero if the date is not included in the input
string.  Null pointers may be supplied for any unwanted return date and
time parameters.
...
fits\_parse\_extnum   & \label{ffextn} \\
