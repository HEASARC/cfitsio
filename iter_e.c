...
*/
    ...
    cols[0].fptr = fptr;
    cols[0].datatype = TLONG;   /* define the desired datatype */
    cols[0].iotype = OutputCol;    
...
*/
{
    ...
    char filename[]  = "events.fit";     /* name of FITS table */
...
*/
{
    ...
    if (firstrow == 1)
    {
       ...
       if (cols[0].datatype != TLONG || cols[1].datatype != TLONG)
         return(-4);  /* wrong datatypes */

       /* assign the input array points to the X and Y arrays */
       xcol = (long *) cols[0].array;
       ycol = (long *) cols[1].array;
