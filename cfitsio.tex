...
The CFITSIO code is contained in about 30 source files (*.c) and 3 header
...
on other systems, at least with cc.

The only difference between the shared library and the .a library is
that the shared library isn't copied into your program at link time.
Rather, the program has to find it at run time, normally through
...
PPCLibrary.(pi) into CodeWarrior Pro 1 and make.  This builds the
cfitsio library for PPC.  There are also targets for both the test
program and the speed test program..
...
1.  Read the following `FITS Primer' chapter for an
overview of the structure of FITS files.  This is especially important
for users who are unfamiliar with the FITS table and image
extensions.
...
is included with the CFITSIO distribution for other examples.
...
read or write information in this fitsfile structure because the
definition of the structure may change in future versions of CFITSIO.
The cookbook.c program that is included with the CFITSIO library
contains example programs illustrating this usage.


\section{File Access: on Disk and in Memory}

In addition to reading or writing traditional FITS files or
compressed FITS files on magnetic
disk, CFITSIO can also access FITS files in computer memory and can
read or write FITS files that are piped to the `stdin' and `stdout'
file streams.  The use of these features is outlined in the following
sections.
...
can be much faster than for magnetic disk files, this feature of CFITSIO
can be used to reduce the I/O processing times when dealing with FITS
files as long as the machine has enough memory to hold the entire
file.
...
\section{Defined Symbolic Constants}

A number of symbolic constants have been defined within CFITSIO (in
fitsio.h) for the convenience of application programmers.  Use of these
symbolic constants, rather than the actual numeric value, may help to make
the source code more readable and easier for others to understand.
...
The fits\_write\_key\_unit/ffpunt and fits\_read\_key\_unit/ffgunt
routines in CFITSIO write and read, respectively, the keyword unit
...
dimensions of the primary array or IMAGE extension. Table extensions may have
...
\item[1 ] Open an existing FITS file with readonly or readwrite access.
    ...
    READONLY or READWRITE.  Compressed FITS files (Unix compress,
   gzip, or pkzip format) may be opened with READONLY mode.
\end{description}
...
\item[4 ] Move to a specified (absolute) HDU in the FITS file.  When a FITS file
    ...
    HDU (the primary array) in the file which has hdunum = 1.  If the
    returned hdutype value is not desired, one may enter a null pointer for
   that parameter.
\end{description}
...
parameters pointed to by the `array` and  `nulval' pointers.
`Datatype'  can have one of the following symbolic constant values:
TBYTE, TUSHORT, TSHORT, TINT, TULONG, TLONG, TFLOAT, or TDOUBLE.  The
routines for reading or writing table columns also support the
TLOGICAL, TSTRING, TCOMPLEX and TDBLCOMPLEX datatypes. Automatic
datatype conversion will be attempted for numerical datatypes if the
specified datatype is different from the actual datatype of the FITS
array or table column.
...
\item[1 ] Open an existing FITS file with readonly or readwrite access.
    ...
    (see fits\_set\_mem\_buff, below) then a null file name may be given to
...
\begin{description}
\item[2 ]  Create and initialize a new empty FITS file.  If the filename
     ...
     (see fits\_set\_mem\_buff, below) then the file name  is ignored
...
\begin{description}
\item[3 ]Return the name of the FITS file
\end{description}
...
\begin{description}
\item[4 ]Return the I/O mode of the FITS file (READONLY or READWRITE)
\end{description}
...
\begin{description}
\item[5 ]Flush any internal buffers of data to the output FITS file. This
...
\begin{description}
\item[6 ]Close a previously opened FITS file.  If the file resides in a
   user-allocated memory buffer
   (see fits\_set\_mem\_buff, below) then the memory buffer is left unchanged
...
\begin{description}
\item[7 ] Close and DELETE a FITS disk file previously opened with ffopen or ffinit.
...
\begin{description}
\item[8 ] Initialize a memory buffer to be used for reading or writing a FITS file
...
\begin{description}
\item[9 ] Copy a FITS file residing in memory (see fits\_set\_mem\_buff) out to
...
\item[2 ] Move a relative number of HDUs forward or backwards in the FITS file.
    If the returned hdutype value is not desired, one may enter a null
...
\begin{description}
\item[3 ] Move to the HDU which has the specified EXTNAME and EXTVERS keyword
    values.  Setting extvers = 0 will move to the first HDU with
    the given extname and will ignore the version number.  If no matching
    HDU is found in the file, the current HDU will remain unchanged
   and a status = BAD\_HDU\_NUM will be returned.
\end{description}

\begin{verbatim}
  int fits_movnam_hdu / ffmnhd
      (fitsfile *fptr, char *extname, int extvers, > int *status)
\end{verbatim}
...
\begin{description}
\item[5 ] Get the type of the current HDU in the FITS file.  The allowed
    values for hdutype are: IMAGE\_HDU (0), ASCII\_TBL (1), or
   BINARY\_TBL (2).
\end{description}
...
\begin{description}
\item[6 ] Get the total number of HDUs in the FITS file by scanning through
   the whole file.  The CHDU remains unchanged.
\end{description}
...
\begin{description}
\item[7 ] Get the byte offsets in the file to the start and end of the CHDU.
...
\begin{description}
\item[8 ]Create a new primary array if a new empty FITS file has just been
   created, otherwise append a new IMAGE extension following the current HDU.
...
\begin{description}
\item[9 ]Create (append) a new table extension following the current HDU.
...
\begin{description}
\item[10] Create (append) a new empty HDU following the last extension that
...
\begin{description}
\item[11]  Insert a new IMAGE extension immediately following the CHDU.
...
\begin{description}
\item[12] Insert a new ASCII TABLE extension immediately following the CHDU.
    Any following extensions will be shifted down to make room for
    the new extension.  If there are no other following extensions
...
\begin{description}
\item[13] Insert a new binary table extension immediately following the CHDU.
    Any following extensions will be shifted down to make room for
    the new extension.  If there are no other following extensions
...
\begin{description}
\item[14] Modify the size, dimensions, and/or datatype of the current
...
\begin{description}
\item[15] Delete the CHDU in the FITS file.  Any following HDUs will be shifted
...
\begin{description}
\item[16] Copy the entire CHDU from the FITS file associated with infptr to the CHDU
...
\begin{description}
\item[17] Copy the data (and not the header) from the CHDU associated with infptr
...
\begin{description}
\item[18] This routine forces CFITSIO to scan the current header keywords that
...
\subsection{ Write or Copy Keyword Routines  \label{FFPREC}}
...
\begin{description}
\item[8 ] Put (append) a new keyword with an undefined, or null, value into the CHU.
...
\begin{description}
\item[9 ] Put (append) a numbered sequence of keywords into the CHU.   One may
...
\begin{description}
\item[10] Copy an indexed keyword from one HDU to another, modifying
...
\begin{description}
\item[11] Put (append) a `triple precision' keyword into the CHU in F28.16 format.
...
\begin{description}
\item[8 ] Get a sequence of indexed keyword values.  If the value of any of
    the keywords is not defined (i.e., the value field is blank) then an
    error status = VALUE\_UNDEFINED (204) will be returned and the input
...
particular X, Y coordinate in the image.  C programmers should
note that the ordering of arrays in FITS files, and hence in
all the CFITSIO calls, is more similar to the dimensionality
of arrays in Fortran rather than C.  For instance if a FITS
image has NAXIS1 = 100 and NAXIS2 = 50, then a 2-D array
just large enough to hold the image should be declared
as array[50][100] and not as array[100][50].
...
\item[1 ] Get elements from the data array.  The `datatype' parameter specifies
    the datatype of the  parameters pointed to by the `nulval'  and `array'
    pointers; it
...
ffgcnn also returns the name of the column.  If casesen = TRUE = CASESEN then
the column name match will be case-sensitive, whereas if casesen = FALSE =
CASEINSEN then the case of the letters will be ignored.
...
\item[2 ] Get the datatype of a column in an ASCII or binary table.  This routine
    ...
    repeat count (which is always 1 for ASCII table columns) is also returned.
...
\subsection{ Insert, Delete, or Copy Rows and Columns Routines \label{FFIROW}}
...
\item[5 ] Copy a column from one HDU to another (or to the same HDU).  If
    ...
    be overwritten (in which case it must have an appropriate datatype).
...
\item[1 ] Put elements into an ASCII or binary table column (in the CDU).
   ...
   datatypes: TLOGICAL, TCOMPLEX, and TDBLCOMPLEX.
\end{description}
...
\begin{description}
\item[4 ] Put string elements into a binary table column (in the CDU)
...
\begin{description}
\item[5 ] Put logical elements into a binary table column (in the CDU)
    substituting the FITS null value for any elements that
    have the corresponding value of nularray = TRUE.
   This routine must NOT be used to write to  variable length array columns.
\end{description}

\begin{verbatim}
  int fits_write_colnul_log / ffpcnl
      (fitsfile *fptr, int colnum, long firstrow, long firstelem,
       long nelements, char *array, char *nularray, > int *status)
\end{verbatim}

\begin{description}
\item[6 ] Put bit values into a binary byte ('B') or bit ('X') table column (in the
...
\begin{description}
\item[7 ] Set table elements in a column as undefined
\end{description}
...
\begin{description}
\item[8 ] Put the descriptor for a variable length column in a binary table.
...
\begin{description}
\item[9 ] Define the zero indexed byte offset of the 'heap' measured from
...
\item[1 ] Get elements from an ASCII or binary table column (in the CDU).  The
    `datatype' parameter specifies the datatype of the  parameters pointed to by
    the `nulval'  and `array'  pointers; it can have one of the following
    symbolic constant values:  TSTRING, TBYTE, TSHORT, TUSHORT, TINT, TLONG,
    TULONG, TFLOAT, or TDOUBLE.  Binary tables also support the following
...
\begin{description}
\item[2 ] Get the current system data (provided for compatibility with
...
\begin{description}
\item[18] Check that the Header fill bytes (if any) are all blank.
\end{description}
...
\begin{description}
\item[19] Check that the Data fill bytes (if any) are all zero or blank.
\end{description}
...
\begin{verbatim}
  ...
  int ffflmd(fitsfile *fptr, > int *iomode, int *status)
...
\begin{verbatim}
  ...
  int ffmnhd(fitsfile *fptr, char *extname, int extvers, > int *status)
  int ffghdn(fitsfile *fptr, > int *hdunum)
  int ffghdt(fitsfile *fptr, > int *hdutype, int *status)
...
\begin{verbatim}
  ...
  int ffpkne[edfg](fitsfile *fptr, char *keyroot, int nstart, int nkey,
             DTYPE *numval, int decimals, char **comment, > int *status)
  int ffcpky(fitsfile *infptr, fitsfile *outfptr, int innum, int outnum,
             char *keyroot, int *status)
...
\begin{verbatim}
  ...
  int ffptbb(fitsfile *fptr, long firstrow, long firstchar, long nchars,
           unsigned char *values, > int *status)
\end{verbatim}
 Insert, Delete, or Copy Rows and Columns Routines: page~\pageref{FFIROW}
...
\begin{verbatim}
  ...
  int ffcpcl(fitsfile *infptr, fitsfile *outfptr, int incolnum, int outcolnum,
            int create_col, int *status);
\end{verbatim}
...
\begin{verbatim}
  ...
  int ffgcfs(fitsfile *fptr, int colnum, long firstrow, long firstelem,
           long nelements, > char **array, char *nularray, int *anynul,
           int *status)
  int ffgcf[l,b,i,ui,j,uj,k,e,d,c,m](fitsfile *fptr, int colnum, long firstrow,
           long firstelem, long nelements, > DTYPE *array,
           char *nularray, int *anynul, int *status)
