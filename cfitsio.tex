...
software, such as ftp-os2.nmsu.edu/pub/os2/dev/emx/v0.9c and
...
\section{Disk Files, Memory-Resident Files, and Piped Files}

In addition to reading or writing traditional FITS files on magnetic
...
\begin{description}
\item[4 ]Close a previously opened FITS file.  If the file resides in a
...
\begin{description}
\item[5 ] Close and DELETE a FITS disk file previously opened with ffopen or ffinit.
...
\begin{description}
\item[6 ] Initialize a memory buffer to be used for reading or writing a FITS file
...
\begin{description}
\item[7 ] Copy a FITS file residing in memory (see fits\_set\_mem\_buff) out to
...
\begin{description}
\item[4 ] Get the byte offsets in the file to the start and end of the CHDU.
...
\begin{description}
\item[11] Modify the size, dimensions, and/or datatype of the current
...
\begin{description}
\item[13] Copy the entire CHDU from the FITS file associated with infptr to the CHDU
...
\begin{description}
\item[14] Copy the data (and not the header) from the CHDU associated with infptr
...
\begin{description}
\item[15] This routine forces CFITSIO to scan the current header keywords that
...
\begin{description}
\item[11] Append the physical units string to an existing keyword.  This
...
\begin{description}
\item[2 ] Return a descriptive text string corresponding to a CFITSIO error
...
\begin{description}
\item[4 ] Write an 80-character message to the CFITSIO error stack.  Application
...
\begin{description}
\item[5 ]  Clear the entire error message stack.  This routine is useful
...
\begin{description}
\item[6 ] Convert a character string to uppercase (operates in place).
\end{description}
...
\begin{description}
\item[7 ]  Compare the input template string against the reference string
...
\begin{description}
\item[8 ] Test that the keyword name contains only legal characters: A-Z,0-9,
...
\begin{description}
\item[9 ] Parse a header keyword record.
...
\begin{description}
\item[13] Parse the 'TFORM' binary table column format string.
...
\begin{description}
\item[14] Parse the 'TFORM' keyword value that defines the column format in
...
\begin{description}
\item[15] Calculate the starting column positions and total ASCII table width
...
\begin{description}
\item[16] Parse a template string and return a formatted 80-character string
