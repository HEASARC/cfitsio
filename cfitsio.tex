...
6 terabytes in size.  It is also required that the value of the NAXISn
...
\item[8 ] Read a rectangular subset from the FITS data array.   Undefined FITS array
    ...
    undefined pixels will be performed.  This routine supports reading
   Large images which may contain more than 2**31 = 2.1E9 pixels. \label{ffgsv}
\end{description}
 
\begin{verbatim}
  int fits_read_subset / ffgsv
      (fitsfile *fptr, int  datatype, long *fpixels, long *lpixels, long *inc,
       DTYPE *nulval, > DTYPE *array, int *anynul, int *status)
\end{verbatim}
 
\begin{description}
\item[9 ] Read elements from the FITS data array.   Undefined FITS array
    elements will be returned with a value = *nullval, (note that this
    parameter gives the address of the null value, not the null value
    itself) unless nulval = 0 or *nulval = 0, in which case no checks for
...
\begin{description}
\item[10]  Read elements from the FITS data array.   Any undefined FITS array
