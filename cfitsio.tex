...
format.  This
library is written in ANSI C and provides a powerful yet simple interface for
accessing FITS files which will run on most commonly used computers and
workstations. CFITSIO supports all the features
...
Makefile for the particular system, then the `make' command compiles
the source files and builds the library.

An alternate makefile called Makefile.shared is included in the
distribution that will build CFITSIO as a shared library.  It
has only been tested under Solaris 2.5 but will likely also work
on other systems, at least with cc.  When using the shared library
the executable code is not copied into your program at link time
and instead the program locates the necessary library code at run
time, normally through
LD\_LIBRARY\_PATH or some other method.  The advantages are:
...
\begin{verbatim}
   ...
   3.  Possibly easier maintenance since you might be able to update the
       shared library without relinking all the tools.
...
2. Review the various topics discussed in Chapter 4 to become familiar
with the conventions and advanced features of the CFITSIO interface.
...
in the `Advanced Interface', as described in Chapter 6.  These routines
...
\section{File Access: on Disk and in Memory}
...
\subsection{Compressed FITS Files}
...
\subsection{Reading or Writing Memory-Resident FITS Files}
...
section 7.1.  This routine is used to tell CFITSIO the initial address
...
application program is responsibility for freeing the memory once it is
no longer needed.
...
\subsection{Piped FITS Files via STDIN and STDOUT.}
...
\begin{verbatim}
      ...
      colnum    = 2;
      fits_write_col(fptr, TULONG, colnum, firstrow, firstelem,
...
Alternatively, one may use the fits\_insert\_rows routine to insert
additional rows into a table if the original value turns out to be too
small.  This routine automatically updates the NAXIS2 value.  This
latter method must be used if any of the columns contain variable
length arrays.


\section{The CFITSIO Iterator Function}
...
function which then reads the data and passes it to the users work
function.
...
\subsection{The Iterator Work Function}
...
The totaln, offset, narrays, cols, and userPointer parameters are
...
character string, the array point is actually a pointer to an array of
string pointers (i.e., **array); The remaining structure elements are
provided for convenience in case that information is need within the
work function.
...
\subsection{The Iterator Driver Function}
...
\subsection{Guidelines for Using the Iterator Function}
...
write out a 2D histogram image.  That work function in turn calls
...
1. Each function has 2 names: a long descriptive name and a short
...
\section{CFITSIO Error Status Utility Routines}
...
information about the opened FITS file.  CFITSIO can open ordinary
FITS files as well as FITS files that have been compressed with the
gzip, Unix compress, or pkzip algorithms (with READONLY mode only).
Additionally, if the filename is specified as ``-'' (a minus sign) then
the file will be read from the ``stdin'' file stream when opening a
file, or written to the ``stdout'' stream when creating a new file.
...
\begin{description}
\item[2 ]  Create a new empty FITS file.
\end{description}
...
\begin{description}
\item[3 ]  Create a new FITS file, using a template file to define its
  initial size and structure.  The template may be another FITS file
...
\begin{description}
\item[5 ]Return the name of the opened FITS file.
\end{description}
...
\begin{description}
\item[6 ]Return the I/O mode of the open FITS file (READONLY or READWRITE).
\end{description}
...
\begin{description}
\item[1 ] Move to a specified absolute HDU number in the FITS file.  When a FITS
...
\item[2 ] Move a relative number of HDUs forward or backwards in the FITS file
   ...
   parameter if it's value is not needed.
\end{description}
...
\item[3 ] Move to the (first) HDU which has the specified extension type and
   ...
   and a status = BAD\_HDU\_NUM will be returned.
\end{description}
...
\item[5 ] Return the type of the current HDU in the FITS file.  The possible
   values for hdutype are: IMAGE\_HDU, ASCII\_TBL, or BINARY\_TBL.
\end{description}
...
\item[8 ]Create a new ASCII or bintable table extension. If
   ...
   values of ASCII\_TBL or BINARY\_TBL.  The tunit and extname parameters
...
\begin{description}
\item[2 ] Write a keyword of the appropriate datatype into the CHU.
    This routine will modify the value and comment field if the
    keyword already exists in the header, otherwise it will append
    a new keyword at the end of the header. Note that the address
    to the value, and not the value itself must be entered.
    The datatype parameter specifies the datatype of the keyword value
    and can have one of the following values:  TSTRING,
   ...
   the comment field blank (or unmodified).
\end{description}
...
\begin{description}
\item[3 ] Write a keyword with a null or undefined value (i.e., the value
...
\begin{description}
\item[4 ] Write (append) a COMMENT keyword to the CHU.  The comment string will
...
\begin{description}
\item[5 ]Write (append) a HISTORY keyword to the CHU.  The comment string will
...
\begin{description}
\item[6 ] Write the DATE keyword to the CHU. The keyword value will contain
    ...
    simply update the keyword value with the current date.  A new
    4-digit year date format has been approved to solve the Year-2000
   problem and will be implemented in CFITSIO in late 1998.
\end{description}
...
\begin{description}
\item[7 ]Write a user specified keyword record into the CHU.  This is
...
\begin{description}
\item[10]  Modify (overwrite) the comment field of an existing keyword.
\end{description}
...
\begin{description}
\item[11] Read the nth header record in the CHU.  The first keyword
...
\begin{description}
\item[12]  Read the header record having the specified keyword name.
\end{description}
...
\begin{description}
\item[13] Read a specified keyword value and comment.
    ...
    values:  TSTRING, TLOGICAL, TBYTE, TSHORT, TUSHORT, TINT, TLONG,
...
\begin{description}
\item[14] Read the physical units string in an existing keyword.  This
    routine uses a local convention, shown in the following example,
    in which the keyword units are enclosed in square brackets in the
...
\begin{description}
\item[15] Delete a keyword record.  The space previously occupied by
...
\item[1 ]  Iterator `constructor' functions that set
   ...
   that define the columns or arrays. These set the fitsfile
    ...
    respectively.  The last 2 routine allow all the parameters
...
\begin{description}
\item[3 ]  The CFITSIO iterator function
\end{description}
...
TSHORT, TUSHORT, TINT, TLONG, TULONG, TFLOAT, TDOUBLE.  Automatic data
...
\begin{description}
\item[2 ]Write elements into the FITS data array, substituting the appropriate
  ...
  ignored and this routine behaves the same as fits\_write\_img.
\end{description}
...
\begin{description}
\item[3 ]Set FITS data array elements equal to the appropriate null
...
\begin{description}
\item[4 ] Read elements from the FITS data array.   Undefined FITS array
...
\section{FITS ASCII and Binary Table Routines}
...
matches an input template name.   If casesen  = CASESEN then
the column name match will be case-sensitive, whereas if casesen  =
CASEINSEN then the case will be ignored.  As a general rule, the
column names should be treated as case INsensitive.
...
\begin{description}
\item[2 ] Return the datatype and vector repeat value for a column in an ASCII or
    ...
    The negative of the datatype code value is
...
\subsection{Routines to Insert, Delete, or Copy Rows and Columns \label{FFIROW}}


\begin{description}
\item[1 ] Insert blank rows into an ASCII or binary table.
...
\begin{description}
\item[3 ] Insert a blank column (or columns) into an ASCII or binary
...
\begin{description}
\item[4 ] Delete a column from an  ASCII or binary table (in the CDU).
...
\begin{description}
\item[5 ] Copy a column from one HDU to another (or to the same HDU).  If
...
or binary table extension.  Automatic data type conversion is performed
...
TSHORT, TUSHORT, TINT, TLONG, TULONG, TFLOAT, or TDOUBLE.  Binary
...
\item[2 ] Write elements into an ASCII or binary table column,
   ...
   This routine must not be used to write to variable length array columns.
\end{description}
...
\chapter{ Advanced CFITSIO Interface Routines }
...
\section{Advanced FITS File Access Routines}


\begin{description}
\item[1 ]Flush any internal buffers of data to the output FITS file. This
...
\begin{description}
\item[2 ] Close and DELETE a FITS disk file previously opened with ffopen or ffinit.
...
\begin{description}
\item[3 ] Initialize a memory buffer to be used for reading or writing a FITS file
in computer memory.  This routine must be called before calling
fits\_open\_file or fits\_create\_file to tell CFITSIO that the file
...
'mem\_size' gives the initial size of the block of memory.
'Mem\_realloc' is a pointer to an optional function that CFITSIO can
...
'delta\_size' parameter may be used to suggest a minimum amount of
...
whichever is larger.  Values of delta\_size less than 2880 will be
...
and/or specifying a larger delta\_size value may help to reduce the
...
passed on as input to the fits\_open\_file or fits\_create\_file
...
\begin{description}
\item[4 ] Copy a FITS file residing in memory (see fits\_set\_mem\_buff) out to
    a disk file.  The memory may be copied out to the 'stdout' stream
    by giving a filename = "-".  The 'mem\_size' parameter specifies the
...
\section{Advanced HDU Access Routines}


\begin{description}
\item[1 ] Get the byte offsets in the FITS file to the start and end of the CHDU.
    The difference between these 2 values gives the size of the CHDU.
    If the CHDU is the last HDU in the file, then the ending offset
...
\begin{description}
\item[7 ] This routine forces CFITSIO to scan the current header keywords that
    ...
    useful for reinitializing the structure of an HDU
    if any of the required keywords (e.g., NAXISn) have been modified.
...
\section{Advanced Header Keyword Routines}
...
check to see if a keyword with the same name already exists.


\begin{description}
\item[1 ] Write (append) a new keyword of the appropriate datatype into the CHU.
    The datatype parameter specifies the datatype of the keyword value
    with one of the following values:  TSTRING,
    TLOGICAL, TBYTE, TSHORT, TUSHORT, TINT, TLONG, TULONG, TFLOAT, TDOUBLE.
    A null pointer may be entered for the comment parameter which will cause
   the comment field to be left blank.
\end{description}
...
\begin{description}
\item[2 ] Write (append) a new keyword with an undefined, or null, value into the CHU.
...
\begin{description}
\item[3 ]  Write (append) a new keyword of the appropriate datatype into the CHU.
...
\begin{description}
\item[4 ] Write (append) a string valued keyword into the CHU which may be longer
...
\begin{description}
\item[5 ] Write (append) a numbered sequence of keywords into the CHU.   One may
...
\begin{description}
\item[6 ]Copy an indexed keyword from one HDU to another, modifying
...
\begin{description}
\item[7 ]Write (append) a `triple precision' keyword into the CHU in F28.16 format.
...
\begin{description}
\item[8 ]Write keywords to the CHDU that are defined in an ASCII template file.
...
These insert routines are somewhat less efficient than the write keyword
routines described above because the following keywords in the header must
...
\subsection{ Advanced Read Keyword Routines}

Wild card characters
may be used when specifying the name of the keyword to be read.
...
\begin{description}
\item[5 ] Read a sequence of indexed keyword values.  If the value of any of
...
\subsection{ Advanced Update Keyword Routines \label{FFUCRD}}
...
\section{Advanced FITS Primary Array or IMAGE Extension I/O Routines}
...
being written or read from the FITS array.
...
\begin{description}
\item[1 ] Write elements into the data array.  The datatype is specified
...
\begin{description}
\item[2 ]Write elements into the data array, substituting the appropriate FITS null
...
\begin{description}
\item[4 ] Write values into group parameters.  This routine only applies
...
\begin{description}
\item[5 ] Write a 2-D image into the data array.
\end{description}
...
\begin{description}
\item[7 ] Write a 3-D cube into the data array.
\end{description}
...
\begin{description}
\item[8 ]  Write an arbitrary data subsection into the data array.
\end{description}
...
\begin{description}
\item[9 ] Read elements from the data array.  Undefined array elements will be
...
\begin{description}
\item[10]  Read elements and nullflags from data array.
...
\begin{description}
\item[11]  Read values from group parameters.  This routine only applies
    to the `Random Grouped' FITS format which has been used for
    applications in radio interferometry, but is offically deprecated
   for future use.
\end{description}
...
\begin{description}
\item[12]   Read 2-D image from the data array.  Undefined
...
\begin{description}
\item[13]  Read 3-D cube from the data array.   Undefined
    pixels in the array will be set equal to the value of 'nulval',
    unless nulval=0 in which case no testing for undefined pixels will
   be performed.
\end{description}
...
\begin{description}
\item[14]    Read an arbitrary data subsection from the data array.  Undefined
       pixels in the array will be set equal to the value of 'nulval',
...
\begin{description}
\item[15]    Read an arbitrary data subsection from the data array.  Any Undefined
...
\section{Advanced FITS ASCII and Binary Table Routines}
...
\begin{description}
\item[3 ] Write (append) a TDIMn keyword whose value has the form '(l,m,n...)'
...
\begin{description}
\item[4 ] Return the number of and size of the dimensions of a table column.
...
\begin{description}
\item[5 ] Return optimal number of rows to read or write at one time for
...
\subsection{Advanced Write Column Data Routines}
...
\begin{verbatim}
  int fits_write_colnul_[log, byt, sht, usht, lng, ulng, int, flt, dbl] /
      ffpcn[l,b,i,ui,j,uj,k,e,d]
      (fitsfile *fptr, int colnum, long firstrow, long firstelem,
       long nelements, DTYPE *array, DTYPE nulval, > int *status)
\end{verbatim}
...
\item[3 ] Write string elements into a binary table column (in the CDU)
    ...
    are equal to the nulstr string.
   This routine must NOT be used to write to  variable length array columns.
\end{description}
...
\begin{description}
\item[4 ] Write bit values into a binary byte ('B') or bit ('X') table column (in the
...
\subsection{Advanced Read Column Data Routines}
...
\begin{verbatim}
  ...
  int fits_read_col_[log,byt,sht,usht,lng,ulng, int, flt, dbl, cmp, dblcmp] /
      ffgcv[l,b,i,ui,j,uj,k,e,d,c,m]
      (fitsfile *fptr, int colnum, long firstrow, long firstelem,
...
\begin{verbatim}
  ...
  int fits_read_colnull_[log,byt,sht,usht,lng,ulng, int, flt, dbl, cmp, dblcmp] /
      ffgcf[l,b,i,ui,j,uj,k,e,d,c,m]
      (fitsfile *fptr, int colnum, long firstrow,
...
\begin{description}
\item[6 ] Return the descriptor for a variable length column in a binary table.
...
\section{Advanced General Utility Routines \label{FFVERS}}
...
\begin{description}
\item[3 ] Write an 80-character message to the CFITSIO error stack.  Application
...
\begin{description}
\item[4 ]  Clear the entire error message stack.  This routine is useful
...
\begin{description}
\item[5 ] Convert a character string to uppercase (operates in place).
\end{description}
...
\begin{description}
\item[6 ]  Compare the input template string against the reference string
...
\begin{description}
\item[7 ] Test that the keyword name contains only legal characters: A-Z,0-9,
...
\begin{description}
\item[9 ] Construct an array indexed keyword name (ROOT + nnn).
...
\begin{description}
\item[11] Determine the datatype of a keyword value string. This routine
...
\item[15] Parse a template string and return a formatted 80-character string
    ...
    string may then be passed to the ffprec, ffmcrd, or fits\_delete\_key routines
...
\begin{description}
\item[16] Check that the Header fill bytes (if any) are all blank.
\end{description}
...
\begin{description}
\item[17] Check that the Data fill bytes (if any) are all zero or blank.
\end{description}
...
datatype - specifies the datatype of the value.  Allowed value are:
           TSTRING, TLOGICAL, TBYTE, TSHORT, TUSHORT, TINT, TLONG, TULONG,
