...
Note that starting with V2.0 of CFITSIO, it may no longer be possible
to build staticly linked executables on some system.  On SUN Solaris,
for example the required nsl library is apparently only available as
a shared library.
...
The development of CFITSIO was made possible through the help of
...
Finally, special thanks go out to the members of the U.S. Congress who in
January 1996 shut down the U.S. government offices, including NASA,
by failing to pass funding legislation, thus giving the author
sufficient free time to write the initial version of CFITSIO.
...
\subsection{Output or Template File Name}

This optional qualifier, which when present is enclosed in parenthesis
immediately following the base file name, serves 2 different purposes
depending on whether the base file is an existing file that is being
opened, or whether is is a new file that is being created:

 
\subsubsection{When Opening an Existing File}

In a number of instances CFITSIO will create a temporary copy in memory
of the input FITS file before it is opened and passed to the
application program.  This happens by default when opening a network
FTP or HTTP-type file, when reading a compressed FITS file on a local
disk, when reading from the stdin stream, or when a column filter, row
filter, or binning specifier is included as part of the input file
specification.  The local machine must then be able to hold a copy of
the whole FITS file in memory, otherwise CFITSIO will exit with an
error.  If this is a problem (e.g., on machines with a small amount of
memory, or when reading very large files) then the output file
specifier may be used to give the name of a local disk file that
CFITSIO will create and use instead of trying to create a copy of the
file in memory.
...
The output file specifier is also useful when reading FTP or HTTP-type
...
\subsubsection{When Creating a New File}
...
all enclosed in square brackets.  If the value of EXTVER and XTENSION
are not specified, then the first extension with the correct value of
EXTNAME is opened. The extension name and type are not case sensitive,
and the extension type may be abbreviated to a single letter (e.g., I =
IMAGE extension or primary array, A or T = ASCII table extension, and B
= binary table BINTABLE extension).   If the HDU location specifier is
equal to `[PRIMARY]' or `[P]', then the primary array (the first HDU)
will be opened.  Examples:
...
column nor keyword with this name exists.
...
evaluates to zero.  For example, $7*(5>3)$ equals 7 whereas $7*(5<3)$ equals
0.
...
a single "\#" as in \#MAXPHA, although this is optional--when a variable
in the expression does not match the name of any of the columns in the
table, the parser will look for a keyword with that same name.  If the
keyword contains a minus sign, which could be confused with an
arithmetic operation then inclose it in "\&" characters as in
\#\&MAX-PHA\&.
...
Here are some examples of valid row filters:
 
\begin{verbatim}
1.    Extract  all  binary  stars brighter than fifth magnitude from a
      FITS file named stars.fits:

            stars.fits[1][binary && mag <= 5.0]

2.    Extract  all  rows between 125 and 175 from a FITS file named
      manyrows.fits:

            manyrows.fits[1][#row >= 125 && #row <= 175]

3.    Extract  all  rows that have the (4,5) component of the IMAGE
      column greater than 100 in a FITS file named vectors.fits:

            vector.fits[1][IMAGE[4,5] .gt. 100]

4.    Extract  all  rows having the absolute value of the sine of theta
      less than a half from a FITS file named angles.fits where the
      angles are tabulated in degrees:

            angles.fits[1][abs(sin(theta * #deg)) < 0.5]

5.    Extract all rows containing a spectrum, held in vector column SPEC,
      with at least one value 3 times  greater than the background level
      held in a keyword, BACKGRND:

            spec.fits[1][SUM( SPEC > 3*BACKGRND )>=1]
\end{verbatim}
...
\item[8 ]Create a new ASCII or bintable table extension. If
   ...
   to reserve space for that many rows, even if a few number of rows will
...
\item[3 ] Write (update) a keyword of the appropriate datatype into the CHU.
    ...
    TLOGICAL (== int), TBYTE, TSHORT, TUSHORT, TINT, TUINT, TLONG, TULONG,
    TFLOAT,
...
\item[15] Read a specified keyword value and comment.
    ...
    TLONG, TULONG, TFLOAT, TDOUBLE, TCOMPLEX, and TDBLCOMPLEX.
...
\begin{description}
\item[3 ] Return the datatype and vector repeat value for a column in an ASCII or
...
\begin{description}
\item[4 ] Write (append) a TDIMn keyword whose value has the form '(l,m,n...)'
...
\begin{description}
\item[2 ]Evaluate an expression on all rows of a table.  If the input and output
...
\begin{description}
\item[3 ] Calculate an expression for the indicated rows of a table, returning
...
\begin{description}
\item[4 ]Evalutate an expression for each row in the input FITS file and place
...
\begin{description}
\item[5 ]Evaluate the given expression and return information on the result. \label{fftexp}
\end{description}
...
\begin{description}
\item[5 ] Parse a header keyword record, returning the value (as
...
\begin{description}
\item[6 ] Construct an array indexed keyword name (ROOT + nnn).
...
\begin{description}
\item[7 ] Construct a sequence keyword name (n + ROOT).
...
\begin{description}
\item[8 ] Determine the datatype of a keyword value string. This routine
...
\begin{description}
\item[9 ] Parse the 'TFORM' binary table column format string.
...
\begin{description}
\item[10] Parse the 'TFORM' keyword value that defines the column format in
...
\begin{description}
\item[11] Calculate the starting column positions and total ASCII table width
...
\begin{description}
\item[12] Parse a template header record and return a formatted 80-character string
...
\begin{description}
\item[13]  Check that the Header fill bytes (if any) are all blank.  These are the bytes
...
\begin{description}
\item[14]  Check that the Data fill bytes (if any) are all zero (for IMAGE or BINARY Table HDU)
...
ffgknx     & \pageref{ffgknx} \\
ffgky          & \pageref{ffgky} \\
ffgkyn         & \pageref{ffgkyn} \\
ffgkyt   & \pageref{ffgkyt} \\
ffgkyx      & \pageref{ffgkyx} \\
...
ffgpfx & \pageref{ffgpfx} \\
ffgpv         & \pageref{ffgpv} \\
ffgpvx     & \pageref{ffgpvx} \\
ffgrec       & \pageref{ffgrec} \\
ffgrsz    & \pageref{ffgrsz} \\
ffgsdt        & \pageref{ffdt2s} \\
ffgsfx & \pageref{ffgsfx} \pageref{ffgsfx2} \\
ffgstm        & \pageref{ffdt2s} \\
ffgsvx  & \pageref{ffgsvx} \pageref{ffgsvx2}\\
...
ffp2dx   & \pageref{ffp2dx} \\
ffp3dx   & \pageref{ffp3dx} \\
ffpcks   & \pageref{ffpcks} \\
ffpcl         & \pageref{ffpcl} \\
ffpcls     & \pageref{ffpcls} \\
ffpclx     & \pageref{ffpclx} \\
ffpcnx & \pageref{ffpcnx} \\
ffpcom      & \pageref{ffpcom} \\
ffpdat         & \pageref{ffpdat} \\
ffpdes  & \pageref{ffpdes} \\
ffpgpx & \pageref{ffpgpx} \\
