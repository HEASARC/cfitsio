...
dozens of people from around the world who have tested the library and
have offered ideas for new features.  The following people in
...
easier to implement.  Jurek Borkowski wrote the Shared Memory
driver, and Bruce O'Neel wrote the drivers for accessing FITS files
over the network using the FTP, HTTP, and ROOT protocols.

Don Jennings designed and wrote the 'Hierarchical Grouping' routines.

Uwe Lamers (XMM/ESA/ESTEC, The Netherlands) designed the
high-performance lexical parsing algorithms that are used in the
on-the-fly filtering of FITS tables.  These algorithms are orders of
magnitude faster than previous implementations for selecting rows from
a table, or for calculating  table column values, based on an arbitrary
boolean or arithmetic expression.  Peter Wilson (RSTX, NASA/GSFC)
provided the final implemention of these lexical parsing routines,
based in part on earlier work by Kent Blackburn (NASA/GSFC),

Peter Wilson also wrote the complete set of Fortran-callable wrappers
for all the CFITSIO routines, which rely on the CFORTRAN macros
...
In addition, many other people have made valuable suggestions or have
helped to test the FITSIO and CFITSIO libraries.  These include (with
apologies to those that I have forgotten to mention):
...
forced the Government offices to shutdown in January 1996 due to a lack
of funding, thus giving the author sufficient free time to write the
initial version of CFITSIO.
...
\begin{description}
\item[9 ] Parse the input filename or URL into its component parts: the file
...
\begin{description}
\item[10]  Parse the input filename and return the HDU number that would be
...
\begin{description}
\item[11] Parse the input file name and return the root file name.  The root
...
(fitsfile *infptr, fitsfile *outfptr, long member, int tfopt, > int *status)
\end{verbatim}
...
\begin{description}
\item[2 ] Create or parse the FITS format date and time string.  Starting in
...
'YYYY-MM-DDThh:mm:ss.ddd...' (date and time) where the number of decimal
...
\begin{description}
\item[5 ] Test that the keyword name contains only legal characters: A-Z,0-9,
...
\begin{description}
\item[14] Parse a template header record and return a formatted 80-character string
...
\begin{description}
\item[15]  Check that the Header fill bytes (if any) are all blank.  These are the bytes
...
\begin{description}
\item[16]  Check that the Data fill bytes (if any) are all zero (for IMAGE or BINARY Table HDU)
