...
\item[3 ] Delete a list of rows from an ASCII or binary table (in the CDU).
    rownum is an array of row numbers to be deleted from the table.
    (The first row in the table has rownum = 1 not 0).  The list of
...
\begin{verbatim}
  int fits_delete_rowlist / ffdrws
      (fitsfile *fptr, long *rownum, long nrows, > int *status)
\end{verbatim}
...
\begin{description}
\item[5 ] Delete a column from an  ASCII or binary table (in the CDU).
...
\begin{description}
\item[6 ] Copy a column from one HDU to another (or to the same HDU).  If
...
\begin{description}
\item[8 ] This routine forces CFITSIO to scan the current header keywords that
...
\begin{description}
\item[5 ] Write (append) a numbered sequence of keywords into the CHU.  The
...
rownum   - sorted list of row numbers to be deleted from the table
