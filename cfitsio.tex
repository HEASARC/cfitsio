...
is included as part of the input file specification.  If there is
not enough memory to create the file copy, then CFITSIO will exit
with an error.   If this is a problem (e.g.,
...
be interpreted the name of the final file as follows, in descending
priority:
...
Because an entire N-dimensional image can be stored in a single cell of
a binary table (i.e. each row of the column contains a different
image), the column name and the row number of the image to be opened
may be given following the HDU specifier, separated by a semi-colon as
shown in the following examples. In this case, the image is copied from
the table cell into a temporary primary array before it is opened.  The
application program then just sees the primary array, without any
extensions.  The particular row to be opened may be specified either by
giving an explicit  integer row number (starting with 1), or by
specifying a boolean expression which evaluates to TRUE for the desired
row.  The first row that satisfies the expression will be used.  The
row selection expression has the same syntax as the Row Filter
Specifier, described in section 4.2.7.
...
System Subroutine library, called 'wcssubs'.  It is available from the
...
any new FITS WCS keyword conventions that may be adopted in the
future.  Thus, software that use these routines is more likely to
remain compatible with the new WCS keyword conventions that are now in
development.

The following 3 routines create a character string array containing
all the WCS keywords that are needed as input to the wcsinit routine.
They provide a convenient interface to the WCS library routines, but
do not actually call any routines in the WCS library themselves.
...
is a 2-dimensional vector and each row of the table contains a
different image. The names of the WCS keywords are translated into the
keywords corresponding to an image (e.g., 1CRPn becomes the CRPIX1
...
\begin{description}
\item[4 ]  This WCS library routine allocates and initializes a structure
containing the values of all the WCS parameters, based on the input
array of header keywords that was generated by any of the three
previous routines. The WorldCoor structure is then used as input to the
following 2 WCS library routines that convert between sky coordinates
...
\begin{verbatim}
  void pix2wcs
       (struct WorldCoor *wcs, double xpix, double ypix,
