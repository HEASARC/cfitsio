...
King, Scott Koch,  Claire Larkin, Rob Managan, Eric Mandel, Richard
Mathar, John Mattox, Carsten Meyer, Emi Miyata, Stefan Mochnacki, Mike
Noble, Oliver Oberdorf, Clive Page, Arvind Parmar, Jeff Pedelty, Tim
Pearson, Maren Purves, Scott Randall, Chris Rogers, Arnold Rots, Barry
Schlesinger, Robin Stebbins, Andrew Szymkowiak, Allyn Tennant, Peter
Teuben, James Theiler, Doug Tody, Shiro Ueno, Steve Walton, Archie
Warnock, Alan Watson, Dan Whipple, Wim Wimmers, Peter Young, Jianjun
Xu, and Nelson Zarate.
...
The CFITSIO library file (libcfitsio.a or libcfitsio.so), and 2
include files, fitsio.h and longnam.h, should then be copied to the
appropriate directory(s) for use when compiling application programs.
...
`extensions'), concatinated one after the other in the file.  The user
...
The 8-byte 'LONGLONG' integer datatype is supported on most platforms.
CFITSIO defines the LONGLONG datatype to be equivalent to 'long long'
on most Unix platforms and on Mac OS-X.  Since most Windows compilers don't
support the 'long long' datatype, LONGLONG is defined instead to be
equivalent to '\_\_int64'.  If the compiler does not support a 8-byte
integer datatype then LONGLONG is defined to be equivalent to 'long'.
...
must be used.  On Windows platform which have the \_\_int64 data type,
...
\section{Support for Unsigned Integers}
...
(currently 40, as defined by NIOBUF in the file fitsio2.h), or by the
...
\begin{description}
\item[5 ]Create a new primary array or IMAGE extension with a specified
...
\begin{description}
\item[6 ]Create a new ASCII or bintable table extension. If
...
\begin{description}
\item[7 ] Copy all or part of the HDUs in the FITS file associated with infptr
...
\begin{description}
\item[8 ] Copy the current HDU from the FITS file associated with infptr and append it
...
\begin{description}
\item[9 ]  Copy the header (and not the data) from the CHDU associated with infptr
...
\begin{description}
\item[1 ] Write a keyword of the appropriate datatype into the
...
\begin{description}
\item[1 ] Get the data type, number of dimensions, and/or the size of
    each dimension in the image .  The datatype is determined by the
    bitpix parameter which has allowed values of BYTE\_IMG (8),
    ...
    maxdim dimensions will be returned).  \label{ffgidt} \label{ffgidm}
   \label{ffgisz} \label{ffgipr}
\end{description}
...
\begin{description}
\item[2 ] Write a rectangular subimage (or the whole image) to the FITS data
...
\begin{description}
\item[3 ] Write pixels into the FITS data array.  'fpixel' is an array of
...
\begin{description}
\item[4 ] Set FITS data array elements equal to the appropriate null pixel
...
\begin{description}
\item[5 ] Read a rectangular subimage (or the whole image) from the FITS
...
\begin{description}
\item[6 ] Read pixels from the FITS data array.  'fpixel' is the starting
   ...
   equal to TRUE.  \label{ffgpxv}  \label{ffgpxf}
\end{description}
...
\begin{description}
\item[2 ] Read elements from an ASCII or binary table column.  The datatype
...
\begin{description}
\item[8 ] Parse a header keyword record, returning the value (as
...
\begin{description}
\item[12] Return the class of an input header record.  The record is classified
...
\begin{description}
\item[16] Parse a template header record and return a formatted 80-character string
...
\begin{description}
\item[17]  Parse the input string containing a list of rows or row ranges, and
...
\begin{description}
\item[18]  Check that the Header fill bytes (if any) are all blank.  These are the bytes
...
\begin{description}
\item[19]  Check that the Data fill bytes (if any) are all zero (for IMAGE or
...
first an initialization routine must be called to read the relevant WCS
...
\section{WCS Routines that require the WCS library}
...
\begin{description}
\item[8 ]Flush any internal buffers of data to the output FITS file. These
...
\begin{description}
\item[1 ] Read a keyword value (with the appropriate datatype) and comment from
...
\begin{description}
\item[2 ] Read a sequence of indexed keyword values.   The starting index number
...
\item
{\tt myfile.fits[EVENTS][PHA > 5]}:  creates and opens a virtural FITS
...
offical FITS Standard definition document, but it is likely that FITS
will eventually support this data type, expecially as computers that
run 64-bit operating systems become more common.  To enable support for
64-bit integers in CFITSIO the \#define statement at the beginning of
the fitsio2.h file must set SUPPORT\_64BIT\_INTEGERS to 1.  Currently
it is set to 0 by default.

The FITS 64-bit images have BITPIX = 64, and the 64-bit binary table
columns have TFORMn = 'K'.  The use of these datatypes on platforms
where the size of a 'long' integer = 8 is rather intuitive.  CFITSIO
will write 'long' variable values to the FITS file and read them back
into 'long' variables just as one would expect.  CFITSIO also supports
implicit data type conversion between 64-bit integer images and columns
and any other supported data type, although some loss of numerical
precision or numerical overflow is likely in this case.

The situation is more difficult on 32-bit computing platforms where the
size of a 'long' variable = 32.  In this case there is no obvious way
to return the full 64 precision of the FITS data values when reading
the FITS file.  CFITSIO will convert the 64-bit integer values into any
other supported data type;  the 64-bit double data type is probably
the most useful in this case.  It only provides about 52-bits of
precision in the mantissa, however, so some lose of precision is
possible.
