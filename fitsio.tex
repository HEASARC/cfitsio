...
\item[1 ] Return the current version number of the fitsio library.
    The version number will be incremented whenever any
...
\item[11]  Parse the input filename and return the HDU number that would be
    moved to if the file were opened with FTNOPN.  If the file name
    just consists of a root name without any extension specifier then
    this routine will return -99 for the value of nhdu.  Note that
    if the filename is specified as 'filename.ext[n]' then nhdu will be
    returned with a value of n+1 because of the difference in the way
    the HDUs are counted in the user interface (starting with 0) and in
   the subroutine interface (starting with 1).
\end{description}
...
\begin{verbatim}
        FTMNHD(unit, hdutype, extname, extvers, > status)
\end{verbatim}
...
\begin{description}
\item[4 ]Put elements into the data array
\end{description}
...
\begin{description}
\item[5 ]Put elements into the data array, substituting the appropriate FITS null
...
\begin{description}
\item[6 ]Set data array elements as undefined
\end{description}
...
\begin{description}
\item[7 ] Get elements from the data array.  Undefined array elements will be
    returned with a value = nullval, unless nullval = 0 in which case no
...
\begin{description}
\item[8 ] Get elements and nullflags from data array.
...
\begin{description}
\item[9 ] Put values into group parameters
\end{description}
...
\begin{description}
\item[10]  Get values from group parameters
\end{description}
